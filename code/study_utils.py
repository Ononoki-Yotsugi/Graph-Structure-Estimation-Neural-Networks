...
def load_data(path, dataset):
    ...
    ind.dataset.allx:
        feature of both labeled and unlabeled training instances (sp.csr.csr_matrix)   #为什么训练样本含有unlabeled的？
    ind.dataset.y:
    ...
    if dataset in ['cora', 'citeseer', 'pubmed']:
        ...
        feature = sp.vstack((allx, tx)).tolil()   #换了一下结构，动机原理未知
        feature = normalize(feature)   #对样本normalize？
        feature[test_idx_reorder,:] = feature[test_idx_range,:]   #这又是在干什么？为什么要把test node的顺序完全换掉？姑且就认为它就这么搞吧，可能与上面那些文件生成的过程有关，总之很辣鸡
        feature = torch.from_numpy(feature.todense()).float()   #feature:(N,feature_dim)
        #print(feature.shape)
        ...
        label = np.where(label)[1]   #又从one-hot换回了普通label
        #print(label)
        ...
        '''
        这里控制了训练中每个类别有多少含标签的训练样本
        默认不会从测试node中抽是吧，但是万一呢？
        下面结果显示正好是0-139，数据肯定先经过了处理
        '''
        #print(np.sort(idx_train))
